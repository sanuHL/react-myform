...
function MyForm() {
  ...
  const handleSubmit = (event) => {
    ...
    validation(inputs.nic, inputs.birth);
  ...
  const validation = (nic, birth) => {
    let yearValue = parseInt(nic.substr(0, 2));
    let tempMonth = parseInt(nic.substr(2, 3));
    ...
    if (yearValue === birthYear && month === birthMonth && date === birthDate) {
